...
<onRefresh>
  ...
  annotation = "Necesită înregistrare pe: http://www.pbxtelecom.ro !";
  redrawDisplay();
</onRefresh>
...
$user=str_replace("|","%5E",$user);
$link="http://romania.smcmobile.ro/list@".$user."%5E1024%5E001?";
$html=file_get_contents($link);
$t=explode(",",$html);
for ($i=0;$i<count($t);$i++) {
if (strpos($t[$i],"@") !== false) break;
$title = $t[$i];
$tit= str_replace(" ","%20",$title).",".$day;
echo '
<item>
<title>'.$title.'</title>
<onClick>
<script>
    showIdle();
    topUrl = "http://127.0.0.1/cgi-bin/scripts/tv/pbx1_curl.php?file='.$tit.'";
    url1= getUrl(topUrl);
    annotation=url1;
    url=url1;
    cancelIdle();
