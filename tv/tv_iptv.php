...
>
    ...
    1=modifica aspect, 2= Add to favorite
		</text>
...
<?php
$l="http://iptv-player.com/?id=database";
$s="http://127.0.0.1/cgi-bin/scripts/util/translate.cgi?stream,,";
$html=file_get_contents($l);
//echo $html;
$videos = explode('status=Working', $html);
...
foreach($videos as $video) {
 ...
 $t4=explode("&",$t3[1]);
 $title=trim($t4[0]);
 if (strpos($link,"rtsp") === false && strpos($link,"m3u8") === false && $title)
 {
 if (preg_match("/mms|rtmp/",$link))
   $link=$s.$link;
  echo '
     <item>
     <title>'.$title.'</title>
