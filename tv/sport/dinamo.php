...
if (userInput == "two" || userInput == "2")
	{
     showIdle();
     url="<?php echo $host; ?>" + "/scripts/tv/sport/dinamo_link.php?file=" + getItemInfo(getFocusItemIndex(),"download");
     movie=getUrl(url);
     cancelIdle();
...
$host = "http://127.0.0.1/cgi-bin";
$html = file_get_contents("http://www.fcdinamo.ro/web/guest/galerii-video");
$videos = explode('td class="imagine_single"', $html);
unset($videos[0]);
...
foreach($videos as $video) {
    ...
    $t2 = explode('"', $t1[1]);
    $link = urlencode(trim($t2[0]));

    $t1 = explode('src="', $video);
    $t2 = explode('"', $t1[1]);
    $image = "http://www.fcdinamo.ro".$t2[0];
    ...
    <script>
    showIdle();
    url="'.$host.'/scripts/tv/sport/dinamo_link.php?file='.$link.'";
    movie=getUrl(url);
    cancelIdle();
