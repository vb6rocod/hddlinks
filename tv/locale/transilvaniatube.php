...
if (userInput == "two" || userInput == "2")
	{
     showIdle();
     url="<?php echo $host; ?>" + "/scripts/tv/locale/transilvaniatube_link.php?file=" + getItemInfo(getFocusItemIndex(),"download");
     movie=getUrl(url);
     cancelIdle();
...
<channel>
	<title>transilvaniatube.ro</title>
	<menu>main menu</menu>
...
include ("../../common.php");
$html = file_get_contents("http://m.transilvaniatube.ro/");
$videos = explode('div class="views-row views-row-', $html);
...
foreach($videos as $video) {
    ...
    $t2 = explode('"', $t1[1]);
    $link ="http://m.transilvaniatube.ro".$t2[0];
    ...
    <script>
    showIdle();
    url="'.$host.'/scripts/tv/locale/transilvaniatube_link.php?file='.urlencode($link).'";
    movie=getUrl(url);
    cancelIdle();
