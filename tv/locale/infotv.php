...
if (userInput == "two" || userInput == "2")
	{
     showIdle();
     url="<?php echo $host; ?>" + "/scripts/tv/locale/infotv_link.php?file=" + getItemInfo(getFocusItemIndex(),"download");
     movie=getUrl(url);
     cancelIdle();
...
$image = "/usr/local/etc/www/cgi-bin/scripts/tv/image/infotv.gif";
...
foreach($videos as $video) {
    ...
    if (strpos($link,"news") !==false) {
        ...
        $title=trim($t4[0]);
        if ($title=="") {
    	   $title=trim(str_between($t1[1],"</strong>","</a>"));
         }
    	$title = trim(preg_replace("/(<\/?)([^>]*>)/e","",$title));
    	$descriere=$title;
    ...
    <script>
    showIdle();
    url="'.$host.'/scripts/tv/locale/infotv_link.php?file='.$link.'";
    movie=getUrl(url);
    cancelIdle();
