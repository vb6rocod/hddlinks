...
if (userInput == "two" || userInput == "2")
	{
     showIdle();
     url="<?php echo $host; ?>" + "/scripts/tv/locale/tvsatrm_link.php?file=" + getItemInfo(getFocusItemIndex(),"download");
     movie=getUrl(url);
     cancelIdle();
...
foreach($videos as $video) {
    ...
    $t1=explode('<div class="postinfo">',$video);
    $t2=explode('|',$t1[1]);
    $data=trim($t2[0]);
    ...
    <script>
    showIdle();
    url="'.$host.'/scripts/tv/locale/tvsatrm_link.php?file='.urlencode($link).'";
    movie=getUrl(url);
    cancelIdle();
