...
$tit = substr(strrchr($search,"/"),1);
$tit=str_replace("-"," ",$tit);
$tit=str_replace("categorie","",$tit);
$tit=trim($tit);
?>
<rss version="2.0">
<onEnter>
...
if (userInput == "two" || userInput == "2")
	{
     showIdle();
     url="<?php echo $host; ?>" + "/scripts/tv/php/sensotv_link.php?file=" + getItemInfo(getFocusItemIndex(),"download");
     movie=getUrl(url);
     cancelIdle();
...
$videos = explode('<span style="display:inline-block; width:100%;', $html);
...
foreach($videos as $video) {
    ...
    $t1 = explode('filename=..', $video);
    $t2 = explode('&', $t1[1]);
    $image = "http://www.sensotv.ro".$t2[0];
...
if (strpos($image,"uploads") !== false) {
    $t1=explode('<span style="font-size',$video);
    $t2=explode(">",$t1[1]);
    $t3=explode('<',$t2[1]);
    $t4=explode("|",$t3[0]);
    $data=trim($t4[1]);
   ...
   $name = preg_replace('/[^A-Za-z0-9_]/','_',$title).".mp4";
    ...
    <script>
    showIdle();
    url="'.$host.'/scripts/tv/php/sensotv_link.php?file='.$link.'";
    movie=getUrl(url);
    cancelIdle();
