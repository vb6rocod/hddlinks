...
if($page) {
		$html = file_get_contents("http://unshowpacatos.antena1.ro/ajax.php?action=showVideosArchive&section=emisiune&limit=10&page=".$page."&offset=0");
} else {
    $page = 1;
		$html = file_get_contents("http://unshowpacatos.antena1.ro/ajax.php?action=showVideosArchive&section=emisiune&limit=10&page=".$page."&offset=0");
}
...
foreach($videos as $video) {
    ...
    $t2 = explode('"', $t1[1]);
    $link = 'http://unshowpacatos.antena1.ro'.$t2[0];

    $t1 = explode(' src="', $video);
    $t2 = explode('"', $t1[1]);
    $image = $t2[0];

    $t1 = explode('Emisiunea din', $video);
    $t2 = explode('</a>', $t1[1]);
    $title = 'Emisiunea din '.$t2[0];

    $link="http://127.0.0.1/cgi-bin/scripts/tv/php/antena1_link.php?file=".urlencode($link);
    $name = preg_replace('/[^A-Za-z0-9_]/','_',"Un show pacatos - ".$title).".mp4";
	echo'
	<item>
	<title>'.$title.'</title>
    <onClick>
