...
$link="http://www.euforia.tv/rubrici/72/cuisine/";
$html = file_get_contents($link);
$l = array();
$img = array();
$tit = array();
$videos = explode('div class="vsthumb hb', $html);
unset($videos[0]);
...
$videos = explode('div id="titleb_', $html);
unset($videos[0]);
$videos = array_values($videos);
foreach($videos as $video) {
  $t1 = explode('class="vsp">', $video);
  $t2 = explode('<', $t1[1]);
  $tit[] = $t2[0];
}
for ($i=0;$i<count($l);$i++) {
    $name = preg_replace('/[^A-Za-z0-9_]/','_',$tit[$i]).".mp4";
    $descriere=$tit[$i];
	echo'
	<item>
	<title>'.$tit[$i].'</title>
    <onClick>playItemURL("'.$l[$i].'", 10);</onClick>
    <download>'.$l[$i].'</download>
    <name>'.$name.'</name>
    <annotation>'.$descriere.'</annotation>
    <image>'.$img[$i].'</image>
    <media:thumbnail url="'.$img[$i].'" />
    <mediaDisplay name="threePartsView"/>
	</item>
	';
}
