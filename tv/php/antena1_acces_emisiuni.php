...
if($page) {
		$html = file_get_contents("http://accesdirect.antena1.ro/ajax.php?action=showVideosArchive&section=emisiune&limit=10&page=".$page."&offset=0");
} else {
    $page = 1;
		$html = file_get_contents("http://accesdirect.antena1.ro/ajax.php?action=showVideosArchive&section=emisiune&limit=10&page=".$page."&offset=0");
}
...
foreach($videos as $video) {
    ...
    $t2 = explode('"', $t1[1]);
    $link = 'http://accesdirect.antena1.ro'.$t2[0];
    ...
    $name = preg_replace('/[^A-Za-z0-9_]/','_',"Acces direct - ".$title).".mp4";
	echo'
	<item>
	<title>'.$title.'</title>
    <onClick>
