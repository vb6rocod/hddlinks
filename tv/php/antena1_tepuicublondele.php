...
<channel>
	...
	<menu>main menu</menu>
<?php
//http://tepuicublondele.a1.ro/paginare/blonde/2/all
$query = $_GET["query"];
if($query) {
...
if($page) {
		$html = file_get_contents("http://tepuicublondele.a1.ro/paginare/blonde/".$page."/all");
} else {
    $page = 1;
    $html = file_get_contents("http://tepuicublondele.a1.ro/paginare/blonde/".$page."/all");
}
...
foreach($videos as $video) {
    ...
    $t1 = explode('<br />', $video);
    $t2 = explode('<br />', $t1[1]);
    $title = $cine.' '.trim($t2[0]);
    $name = preg_replace('/[^A-Za-z0-9_]/','_',"Te pui cu blondele - ".$title).".mp4";
	echo'
	<item>
	<title>'.$title.'</title>
    <onClick>
