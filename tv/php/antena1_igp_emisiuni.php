...
if($page) {
		$html = file_get_contents("http://ingurapresei.antena1.ro/ajax.php?action=showVideosArchive&section=emisiuni&limit=10&page=".$page."&offset=0");
} else {
    $page = 1;
		$html = file_get_contents("http://ingurapresei.antena1.ro/ajax.php?action=showVideosArchive&section=emisiuni&limit=10&page=".$page."&offset=0");
}
...
*/
    $name = preg_replace('/[^A-Za-z0-9_]/','_',"In gura presei - ".$title).".mp4";
	echo'
	<item>
	<title>'.$title.'</title>
    <onClick>
