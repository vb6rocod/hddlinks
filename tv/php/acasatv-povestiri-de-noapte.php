...
if (userInput == "two" || userInput == "2")
	{
     showIdle();
     url="<?php echo $host; ?>" + "/scripts/tv/php/acasatv_link.php?file=" + getItemInfo(getFocusItemIndex(),"download");
     movie=getUrl(url);
     cancelIdle();
...
//http://www2.acasatv.ro/special_shows/povestiri_de_noapte/pagina-2.html
$html = file_get_contents("http://www2.acasatv.ro/special_shows/povestiri_de_noapte/pagina-".$page.".html");
...
$videos = explode('<td height="50" style="background-image', $html);
...
foreach($videos as $video) {
    ...
    $t2 = explode("'", $t1[1]);
    $image = $t2[0];
    $image="/usr/local/etc/www/cgi-bin/scripts/tv/image/acasatv.jpg";
    ...
    <script>
    showIdle();
    url="'.$host.'/scripts/tv/php/acasatv_link.php?file='.$link.'";
    movie=getUrl(url);
    cancelIdle();
