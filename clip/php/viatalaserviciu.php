...
if (userInput == "two" || userInput == "2")
	{
     showIdle();
     url="<?php echo $host; ?>" + "/scripts/clip/php/viatalaserviciu_link.php?file=" + getItemInfo(getFocusItemIndex(),"download");
     movie=getUrl(url);
     cancelIdle();
...
foreach($videos as $video) {
    ...
    $t1=explode('<span class="duration">',$video);
    $t2=explode('<',$t1[1]);
    $durata="Durata:".$t2[0];
    
    $t1=explode('span class="info">',$video);
    $t2=explode('<br',$t1[1]);
    ...
    <script>
    showIdle();
    url="'.$host.'/scripts/clip/php/viatalaserviciu_link.php?file='.$link.'";
    movie=getUrl(url);
    cancelIdle();
