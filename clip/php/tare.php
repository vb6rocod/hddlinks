...
if (userInput == "two" || userInput == "2")
	{
     showIdle();
     url="<?php echo $host; ?>" + "/scripts/clip/php/tare_link.php?file=" + getItemInfo(getFocusItemIndex(),"download");
     movie=getUrl(url);
     cancelIdle();
...
//http://www.tare.ro/recente/pag-2
if($page) {
    $html = file_get_contents("http://www.tare.ro/recente/pag-".$page);
} else {
    $page = 1;
    $html = file_get_contents("http://www.tare.ro/recente");
}
...
include ("../../common.php");
$html = str_between($html,'<ul class="item_list_multi">','</ul>');
...
foreach($videos as $video) {
    ...
    <script>
    showIdle();
    url="'.$host.'/scripts/clip/php/tare_link.php?file='.urlencode($link).'";
    movie=getUrl(url);
    cancelIdle();
