...
>
    ...
    Apăsaţi 2 pentru download, 3 pentru Download Manager
  	...
  	<text  redraw="yes" align="center" offsetXPC="55" offsetYPC="52" widthPC="45" heightPC="5" fontSize="17" backgroundColor="10:105:150" foregroundColor="100:200:255">
...
if (userInput == "two" || userInput == "2")
	{
     showIdle();
     url="<?php echo $host; ?>" + "/scripts/clip/php/zagazaga_link.php?file=" + getItemInfo(getFocusItemIndex(),"download");
     movie=getUrl(url);
     cancelIdle();
...
//http://www.zagazaga.ro/video/funny/pagina2
...
$videos = explode("<div id='rez_caut'>", $html);
...
foreach($videos as $video) {
    ...
    //echo $t2[1];
    $t4=explode("<br/>",$t3[1]);
    $descriere=trim($t4[0].$t4[1]);

    $t1 = explode("src=http", $video);
    $t2 = explode("&", $t1[1]);
    $image = "http".$t2[0];

    $durata=str_between($video,"<div class='duration' style='display:block'>","</div>");
    ...
    <script>
    showIdle();
    url="'.$host.'/scripts/clip/php/zagazaga_link.php?file='.$link.'";
    movie=getUrl(url);
    cancelIdle();
