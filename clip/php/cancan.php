...
if (userInput == "two" || userInput == "2")
	{
     showIdle();
     url="<?php echo $host; ?>" + "/scripts/clip/php/cancan_link.php?file=" + getItemInfo(getFocusItemIndex(),"download");
     movie=getUrl(url);
     cancelIdle();
...
<channel>
	...
	<menu>main menu</menu>
<?php
//http://www.cancan.ro/multimedia/video/exclusiv-16/pagina-1/
//http://www.cancan.ro/multimedia/video/exclusiv-16/
if($page) {
     $page1=$page-1;
     $html = file_get_contents("http://www.cancan.ro/multimedia/video/exclusiv-16/pagina-".$page1."/");
} else {
    $page = 1;
...
$t1=explode('<div class="all_galeries">',$html);
$html=$t1[1];
...
foreach($videos as $video) {
  ...
  $t1 = explode('class="titlu">', $video);
  $t2 = explode('>',$t1[1]);
  $t3 = explode('<', $t2[1]);
  $title = $t3[0];
  
  $t1=explode('class="info">',$video);
  $t2=explode('</div>',$t1[1]);
  $durata = preg_replace("/(<\/?)(\w+)([^>]*>)/e","",$t2[0]);
  $descriere = $title;
  $name = preg_replace('/[^A-Za-z0-9_]/','_',$title).".flv";
    ...
    <script>
    showIdle();
    url="'.$host.'/scripts/clip/php/cancan_link.php?file='.$link.'";
    movie=getUrl(url);
    cancelIdle();
