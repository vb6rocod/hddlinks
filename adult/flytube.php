...
$l="http://www.flytube.com/categories.php";
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, $l);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2 GTB5');
  curl_setopt($ch, CURLOPT_FOLLOWLOCATION  ,1);
  curl_setopt($ch, CURLOPT_REFERER, "http://www.flytube.com/");
  $html = curl_exec($ch);
  curl_close($ch);
$html=str_between($html,'<ul class="col">','</div>');
...
foreach($videos as $video) {
    ...
    $t1=explode('"',$t[1]);
    $link="http://www.flytube.com".$t1[0];
    $link=str_replace("1.html","",$link);
  	$title=str_between($t[1],'>','<');
  	$title=str_replace("More ","",$title);
  	$link=$host."/scripts/adult/php/flytube.php?query=1,".$link;
    echo '
    <item>
    <title>'.$title.'</title>
    <link>'.$link.'</link>
