...
<channel>
	<title>vezifilme.ro - categorii</title>
	<menu>main menu</menu>
...
$html = str_between($html,'title="View all posts filed under Filme"','</ul>');
		$link=$host."/scripts/filme/php/vezifilme.php?query=,http://www.vezifilme.ro/category/filme-noi/";
	  echo '
	  <item>
...
foreach($videos as $video) {
  ...
  $title = trim($t2[0]);
	...
	if (($link <> "") && ($title <> "Cereri Filme")) {
		$link=$host."/scripts/filme/php/vezifilme.php?query=,".$link;
	  echo '
	  <item>
	  <title>'.$title.'</title>
	  <link>'.$link.'</link>
