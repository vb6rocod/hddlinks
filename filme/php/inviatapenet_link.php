...
set_time_limit(60);
$id = $_GET["file"];
$html=file_get_contents("http://inviatapenet.gethost.ro/Play.php?id=".$id);
...
$sub=$t3[0];
$movie=$t2[0];
if (!preg_match("/\.mp4/",$movie) {
$t1=explode("streamer=",$html);
$t2=explode("&",$t1[1]);
$movie=urldecode($t2[0])."&file=".$id;
$t1=explode("captions.file",$html);
$t3=explode('&',$t1[1]);
$sub=$t3[0];
}
$sub = substr(strrchr($sub, "/"), 1);
$sub="http://inviatapenet.gethost.ro/sub/".$sub;
//////////////////////////////////////////////////////////////////
...
if (!$h) {
  $sub="http://hdforall.uphero.com/srt/".$id.".srt";
  $ch = curl_init();
  curl_setopt($ch, CURLOPT_URL, $sub);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.1.2) Gecko/20090729 Firefox/3.5.2 GTB5');
