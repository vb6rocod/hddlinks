...
if($query) {
  ...
  $html = curl_exec($ch);
  curl_close($ch);
$t=explode('td width="670px"',$html);
$t1=explode('src="',$t[1]);
$t2=explode('"',$t1[1]);
$img="http://hdfilm.ro/".$t2[0];
$gen=str_between($html,'Gen:','</p>');
$gen=str_replace("&nbsp;"," ",$gen);
$gen = "Gen: ".trim(preg_replace("/(<\/?)(\w+)([^>]*>)/e","",$gen));
$imdb="Not".trim(str_between($t[1],'<p>Not','</p>'));
$imdb= preg_replace("/(<\/?)(\w+)([^>]*>)/e","",$imdb);

$desc=str_between($html,'class="comment more_s" style="width: 650px;">','</div>');
...
<?php
$html=str_between($html,'ul style="','</ul');
...
foreach($videos as $video) {
   ...
   $link=$host."/scripts/filme/php/hdfilm_ep.php?file=".urlencode($link).",".urlencode($title);
