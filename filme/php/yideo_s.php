...
if($query) {
  ...
  $html = curl_exec($ch);
  curl_close($ch);
$t=explode('<div class="show-details"',$html);
$t1=explode('src=',$t[1]);
$t2=explode('&',$t1[2]);
$img=$t2[0];
$gen=str_between($html,'Genres','</p>');
$gen=str_replace("&nbsp;"," ",$gen);
$gen = "Gen: ".trim(preg_replace("/(<\/?)(\w+)([^>]*>)/e","",$gen));
$imdb="Nota IMDB:".trim(str_between($t[1],'Nota IMDB','</p>'));
$imdb= preg_replace("/(<\/?)(\w+)([^>]*>)/e","",$imdb);

$desc=str_between($html,'class="show-overview">','</div>');
...
>
   ...
   <?php echo $desc; ?>
		...
		<image  redraw="yes" offsetXPC=25 offsetYPC=22.5 widthPC=15 heightPC=20>
        <?php echo $img; ?>
		</image>
...
<?php
//$html=str_between($html,'div class="external-links"','</div');
$t1=explode('div class="external-links"',$html);
$t2=explode("</div",$t1[2]);
$html=$t2[0];
...
foreach($videos as $video) {
   ...
   $link=$host."/scripts/filme/php/yideo_ep.php?file=".urlencode($link).",".urlencode($title);
