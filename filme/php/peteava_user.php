...
$l = "http://www.peteava.ro/".$search."/video/".$page;
  $ch = curl_init();
...
//$videos = explode('"mat_details_container">', $html);
$videos = explode('<div class="video_item">', $html);
unset($videos[0]);
...
foreach($videos as $video) {
  ...
  $t2 = explode('<',$t1[1]);
  $data = "Durata: ".$t2[0];
  $t0=explode('class="mat_details_container"',$video);
  ...
  $t1 = explode('title="', $t0[1]);
  $t2 = explode('"', $t1[1]);
  $title = htmlspecialchars_decode($t2[0]);
  
  $link = $host.'/scripts/filme/php/peteava_link.php?file='.$link.','.urlencode($title);
