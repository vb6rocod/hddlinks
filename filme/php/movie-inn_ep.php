...
$link = "http://www.movie-inn.com/series.php?id=".$queryArr[0];
...
//$cookie="D://movie-inn.dat";
  ...
  $html = curl_exec($ch);
  curl_close($ch);
$t1=explode('<div id="content">',$html);
$image="http://www.movie-inn.com".str_between($t1[1],'src=".','"');
?>
<rss version="2.0">
<onEnter>
  startitem = "middle";
...
$url = $sThisFile."?page=".($page-1).",".urlencode($tit);
?>
<title>Pagina anterioară</title>
...
$videos = explode('episode.php?id=', $html);
unset($videos[0]);
...
foreach($videos as $video) {
  $t2 = explode('"', $video);
  $link=$t2[0];
  //$image="http://www.movie-inn.com".str_between($video,'src=".','"');
  ...
  //$description = str_between($video,'<span class="s_e">','</span>');
    //http://www.movie-inn.com/movie.php?id=439
    ...
    <script>
    showIdle();
    url="http://127.0.0.1/cgi-bin/scripts/filme/php/movie-inn_link.php?file='.$link.',s";
    movie=getURL(url);
    cancelIdle();
