...
$videos=explode('div id="seen',$html);
...
foreach($videos as $video) {
    ...
    $t1=explode('class="episode-title"',$video);
    $t3=explode(">",$t1[1]);
    $t4=explode("<",$t3[1]);
    $title=$t4[0];

    if (strpos($video,"ro.gif") !== false)
     $title=$title." (ro)";

    $link = 'http://127.0.0.1/cgi-bin/scripts/filme/php/yideo_link.php?file='.$link;
