...
$page = $_GET["page"];
if ($page == 1) {
   $l="http://www.dolcetv.ro/filme?c=&f=1&ajaxrequest=1";
} else {
   $l="http://www.dolcetv.ro/filme-pagina-".$page."?c=&f=1&ajaxrequest=1";
}
...
if (strpos($html,"thumbwide") === false) {
$new_file="D://dolce.gz";
...
foreach($videos as $video) {
    ...
    $t2 = explode('"', $t1[1]);
    $link = $t2[0];
    $link = "http://www.dolcetv.ro".str_replace("film-","film2-",$link);
    ...
    $title=str_replace("u00e2","a",$v);
    
    $gratis = str_between($video,'thumb_free">','<');
    //echo $gratis;
    if (($gratis == "Gratuit") && ($title <> "")) {
    ...
    <script>
    showIdle();
    url1=getURL("http://127.0.0.1/cgi-bin/scripts/util/dolce_xml.php?file='.urlencode($link).'");
