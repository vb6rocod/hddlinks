...
{
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/vplay_add.php?mod=add*" + getItemInfo(getFocusItemIndex(),"link1") + "*" + getItemInfo(getFocusItemIndex(),"title1");
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
//http://vplus.ro/channels?s=Stargate+u
$link="http://vplus.ro/channels?s=".$search;
...
$html1 = str_between($html,'class="coll_list">',"</ul>");
...
foreach($videos as $video) {
  ...
  $description = str_between($video,'<span class="s_e">','</span>');
	...
	if ($link <> "") {
		$link = $host."/scripts/filme/php/vplay_sez.php?file=".urlencode($link).",".urlencode($title);
	  echo '
	  <item>
	  <title>'.$title.'</title>
	  <link>'.$link.'</link>
