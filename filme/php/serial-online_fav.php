...
{
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/serial-online_add.php?mod=delete*" + getItemInfo(getFocusItemIndex(),"link1") + "*" + getItemInfo(getFocusItemIndex(),"title1");
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
ret;
  ...
  <channel>
    ...
    <title>serial-online - favorite</title>
...
foreach ($arr as $key => $val) {
  $l=$arr[$key][1];
  $title=$arr[$key][0];
  $link = $host."/scripts/filme/php/serial-online.php?query=".$l.",".urlencode($title);
    echo '
    <item>
    <title>'.$title.'</title>
