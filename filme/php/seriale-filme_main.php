...
{
 showIdle();
 url="http://127.0.0.1/cgi-bin/scripts/filme/php/seriale-filme_add.php?mod=add*" + getItemInfo(getFocusItemIndex(),"link1") + "*" + getItemInfo(getFocusItemIndex(),"title1");
 dummy=getUrl(url);
 cancelIdle();
 redrawDisplay();
 ret="true";
...
<link>'.$host.'/scripts/filme/php/seriale-filme_fav.php</link>
<annotation>Lista serialelor favorite</annotation>
<mediaDisplay name="threePartsView"/>
</item>
';
$l="http://www.seriale-filme.net/tv-shows";
  $ch = curl_init();
...
foreach($videos as $video) {
  ...
  if (sizeof ($t1) > 1 ) {
    ...
    $t2=explode('"',$t1[1]);
    $image=$t2[0];
    $t1=explode('bookmark">',$video);
    $t2=explode('<',$t1[1]);
    $title=trim($t2[0]);
		$link = $host."/scripts/filme/php/seriale-filme.php?query=".$link1.",".urlencode($title);
