...
$videos = explode("document.write(unescape('", $html);
...
foreach($videos as $video) {
    $t2=explode("'",$video);
    $p=urldecode($t2[0]);
    $link=str_between($p,"proxy.link=","&");
    $srt=str_between($p,"captions.file=","&");
    if (strpos($srt,"http") === false && $srt) {
     $t=explode("/",$filelink);
     $srt="http://".$t[2].$srt;
     }
    $title = str_between($link,"://","/");
    if ($link <> "") {
////////
    ...
    movie="http://127.0.0.1/cgi-bin/scripts/filme/php/link.php?file='.urlencode($link).'";
    url=getUrl(movie);
    cancelIdle();
