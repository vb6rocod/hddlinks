...
$url = $sThisFile."?file=".($page-1).",".$search.",".urlencode($tit);
?>
...
$videos = explode('<div class="ic_container">', $html);
...
foreach($videos as $video) {
    ...
    $t2 = explode('"', $t1[1]);
    if (strpos($link, "seenow,ro") === false)
      $link = "http://www.seenow.ro".$t2[0];
    else
      $link=$t2[0];
    ...
    //echo $gratis;
    if (($title <> "") && ($title <> "Oferta Seenow") && (strpos($link,"live") === false)) {
    ...
    <script>
    showIdle();
    url1=getURL("http://127.0.0.1/cgi-bin/scripts/util/seenow_xml.php?file='.urlencode($link).'");
    url="http://127.0.0.1/cgi-bin/scripts/util/translate.cgi?stream," + url1;
    cancelIdle();
