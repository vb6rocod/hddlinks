...
//http://www.watchcartoononline.com/anime/adventures-of-sonic-the-hedgehog
//http://www.watchcartoononline.com/anime/adventures-of-sonic-the-hedgehog/page/2
$link=$search."/page/".$page;
  ...
  $html = curl_exec($ch);
  curl_close($ch);
$t1=explode('div class="katcont">',$html);
$t2=explode('src="',$t1[1]);
$t3=explode('"',$t2[1]);
$image=$t3[0];
$descriere=str_between($t1[1],"<p>","</p>");
...
$url = $sThisFile."?query=".($page-1).",".$search.",".urlencode($tit);
...
<?php } ?>
<?php
$videos = explode('td class="ilbaslik"', $html);
unset($videos[0]);
...
//  imagine  
	if($link!="") {
		$link = "http://127.0.0.1/cgi-bin/scripts/filme/php/filme1_link.php?file=".$link.",".urlencode($titlu);
		echo'
		<item>
		<title>'.$titlu.'</title>
		<link>'.$link.'</link> 
