...
if($query) {
  ...
  $html = curl_exec($ch);
  curl_close($ch);
$t=explode('class="avatar"',$html);
$t1=explode('src="',$t[1]);
$t2=explode('"',$t1[1]);
$img=$t2[0];
$gen=str_between($html,'<div class="slbtm">','</div>');
$gen=str_replace("&nbsp;"," ",$gen);
$gen = "Gen: ".trim($line1 = preg_replace("/(<\/?)(\w+)([^>]*>)/e","",$gen));
$imdb="Premier".trim(str_between($t[1],'Premier','</li>'));

$desc=str_between($html,"</h2>",'</p>');
...
<?php
//$html=str_between($html,'class="seasons"','</ul');
$videos = explode('class="seasonbtn', $html);
...
foreach($videos as $video) {
   ...
   $link=$host."/scripts/filme/php/vezi-online.php?file=".urlencode($link).",".urlencode($title);
